 ...
 * @param {Request} req - Express request object.Excepts a Bearer token in the Authorization header.
...
const authenticate = (req: Request, res: Response, next: NextFunction) => {
  ...
  // Spilt out the token from the 'Bearer' prefix
  const [_, token] = authHeader.split(' ');
  try {
    //Verify the token and extract the useId from the payload
